...
func main() {
	configProvider := config.NewUserHomeConfig("$HOME")
	...
	if appConfig == nil || appConfig.GmailConfig.AccessToken == "" {
		tokens := getGmailCreds()
		appConfig.GmailConfig = config.GmailConfig{
			AccessToken:  tokens.AccessToken,
			RefreshToken: tokens.RefreshToken,
		}
		configProvider.Store(appConfig)
	}

	gmailClient := gmail.NewGmailProvider(appConfig.GmailConfig.AccessToken, appConfig.GmailConfig.RefreshToken)
	gmailClient.GetMail()
}
